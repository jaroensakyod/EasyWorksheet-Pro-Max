...
class WorksheetGenerator:
    ...
    def extract_text_from_file(self, file):
        ...
        try:
            ...
            if file_type == 'pdf':
                ...
                return self._extract_text_from_word(file)
            else:
                return "Error: รองรับเฉพาะ PDF และ Word เท่านั้น"
    ...
    def _extract_text_from_pdf(self, file):
        """สกัดข้อความจาก PDF (รองรับ OCR)"""
        try:
            import PyPDF2
            
            # ลองวิธีปกติก่อน (อ่าน text layer)
            pdf_reader = PyPDF2.PdfReader(file)
            text = ""
            for page in pdf_reader.pages:
                text += page.extract_text() + "\n"
            
            # ถ้าได้ข้อความ ใช้เลย
            if text and text.strip():
                # จำกัดความยาว
                if len(text) > 10000:
                    text = text[:10000] + "...\n[เนื้อหาถูกตัดเหลือ 10,000 ตัวอักษร]"
                return text
            
            # ถ้าไม่ได้ข้อความ → ใช้ OCR
            return self._extract_text_pdf_ocr(file)
    ...
    def _extract_text_pdf_ocr(self, file):
        """สกัดข้อความจาก PDF ด้วย OCR (รูปภาพ/Scan)"""
        try:
            # ตรวจสอบว่าติดตั้ง Tesseract หรือไม่
            import pytesseract
            from PIL import Image
            from pdf2image import convert_from_bytes
            
            # ตรวจสอบ path ของ Tesseract (สำหรับ Windows)
            # ถ้าไม่ได้ติดตั้ง → คืนข้อความแนะนำ
            try:
                pytesseract.get_tesseract_version()
            except:
                return """Error: ไม่พบข้อความใน PDF และ Tesseract OCR ยังไม่ได้ติดตั้ง

วิธีแก้ไข:
1. ดาวน์โหลด Tesseract OCR: https://github.com/UB-Mannheim/tesseract/wiki
2. ติดตั้งและเพิ่ม PATH ในระบบ
หรือใช้ PDF ที่มีข้อความ (ไม่ใช่รูปภาพ/Scan)"""
            ...
            if text.strip():
                ...
                return "Error: ไม่พบข้อความใน PDF (OCR ไม่สามารถอ่านได้)"
                
        except ImportError as e:
            return """Error: ยังไม่ได้ติดตั้ง pytesseract/pdf2image

วิธีแก้ไข:
pip install pytesseract pdf2image
และดาวน์โหลด Tesseract OCR จาก:
https://github.com/UB-Mannheim/tesseract/wiki"""
        except Exception as e:
            return f"Error (OCR): {str(e)}"

    
    def _extract_text_from_word(self, file):
        """สกัดข้อความจาก Word"""
        try:
            ...
            # จำกัดความยาว (สูงสุด 10,000 ตัวอักษร)
            if len(text) > 10000:
                text = text[:10000] + "...\n[เนื้อหาถูกตัดเหลือ 10,000 ตัวอักษร]"
            
            return text if text.strip() else "Error: ไม่พบข้อความใน Word"
        except Exception as e:
            return f"Error: {str(e)}"
    
    def summarize_text(self, text, max_length=2000):
        ...
        try:
            ...
            # ใช้ AI สรุป
            summary_prompt = f"""สรุปข้อความต่อไปนี้ให้กระชับ ไม่เกิน {max_length} ตัวอักษร:

{text}

สรุป:"""
            summary = self.ai.generate(summary_prompt)
            
            if summary and len(summary) > 50:
                return summary
            else:
                # Fallback: ตัดข้อความเดิม
                return text[:max_length] + "...\n[เนื้อหาถูกสรุปเหลือ {max_length} ตัวอักษร]"
        except Exception as e:
            # Fallback: ตัดข้อความเดิม
            return text[:max_length] + "...\n[เนื้อหาถูกตัดเหลือ {max_length} ตัวอักษร]"
...
..."""
    ...
    def create_content_pdf(self, title, school_name, content_type, questions, answers, qr_url=None, logo=None, summary=None, image=None):
        """Create a PDF with worksheet, summary, or quiz"""
        from .exporters import PDFExporter
        return self.pdf_exp.create_content_pdf(
            title, school_name, content_type, questions, answers, 
            qr_url=qr_url, logo=logo, summary=summary, image=image
        )
    
    def create_content_word_doc(self, title, school_name, content_type, questions, answers, summary=None, image=None):
        """Create a Word document with worksheet, summary, or quiz"""
        from .exporters import DocxExporter
        return self.docx_exp.create_content_word_doc(
            title, school_name, content_type, questions, answers,
    ...
    def create_summary_pdf(self, title, school_name, content_type, summary, qr_url=None, logo=None):
        ...
        from .exporters import PDFExporter
        return self.pdf_exp.create_summary_pdf(
            title, school_name, content_type, summary,
            qr_url=qr_url, logo=logo
        )
    
    def create_summary_word_doc(self, title, school_name, content_type, summary):
        """Create a Word document with summary content"""
        from .exporters import DocxExporter
        return self.docx_exp.create_summary_word_doc(
            title, school_name, content_type, summary
        )
    ...
    def create_quiz_pdf(self, title, school_name, content_type, questions, answers, qr_url=None, logo=None):
        ...
        from .exporters import PDFExporter
        return self.pdf_exp.create_quiz_pdf(
            title, school_name, content_type, questions, answers,
            qr_url=qr_url, logo=logo
        )
    
    def create_quiz_word_doc(self, title, school_name, content_type, questions, answers):
        """Create a Word document with quiz content"""
        from .exporters import DocxExporter
        return self.docx_exp.create_quiz_word_doc(
            title, school_name, content_type, questions, answers
        )

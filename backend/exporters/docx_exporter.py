...
class DocxExporter:
    ...
    def create_content_word_doc(self, title, school_name, content_type, questions, answers, summary=None, image=None):
        """Create a Word document with worksheet, summary, or quiz content"""
        doc = Document()
        ...
        type_para = doc.add_paragraph(content_type)
        ...
        # Summary section
        if summary:
            doc.add_heading("สรุปเนื้อหา / Summary", level=1)
            summary_para = doc.add_paragraph(summary)
        
        # Questions/Content section
        if "สรุป" in content_type:
            doc.add_heading("แบบทดสอบความเข้าใจ", level=1)
        else:
            doc.add_heading(content_type, level=1)
